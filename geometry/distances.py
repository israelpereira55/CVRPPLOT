...
def calculate_distance_matrix(coordinates, tsp95_standard=False):
    ...
    for i in range(number_of_lines):
        ...
        for j in range(i, number_of_lines):
            ...
            p2 = coordinates[j]
            if tsp95_standard:
                distances[i][j] = distances[j][i] = int(euclidian_distance(p1, p2) + 0.5)

            else:
                distances[i][j] = distances[j][i] = euclidian_distance(p1, p2) + 0.5
